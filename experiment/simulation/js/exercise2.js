...
function addbtnt24() {
  ...
  if (selfromv2 == "Select") {
    ...
    newIconbtn.setAttribute("onclick", "removerowtr2(this)");
    newIconbtn.setAttribute("style", "cursor:pointer;");
...
function removerowli2(btndel) {

  if (typeof (btndel) == "object") {
    var listItemValue2 = $(btndel).closest("li").data("value");
    $(btndel).closest("li").remove();

    $("#selectfrom2").find('option[value="' + listItemValue2 + '"]').remove();
    $("#selectto2").find('option[value="' + listItemValue2 + '"]').remove();

    /* Remove item from array */
    var index = arrentityt2.indexOf(listItemValue2);
    if (index !== -1) {
      arrentityt2.splice(index, 1);
    }
    console.log(arrentityt2);


    var index = arrprocess2.indexOf(listItemValue2);
    if (index !== -1) {
      arrprocess2.splice(index, 1);
    }
    console.log(arrprocess2);


    var index = arrdatastore2.indexOf(listItemValue2);
    if (index !== -1) {
      arrdatastore2.splice(index, 1);
    }
    console.log(arrdatastore2);
